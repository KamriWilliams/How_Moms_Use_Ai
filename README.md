...
I collected real survey responses from 52 mothers covering their daily AI habits, attitudes, perceived benefits, concerns, and routines. The data was unstructured and inconsistent — which made it a realistic challenge, not a clean Kaggle dataset someone else prepared.
...
### 3. Clustering
Before running KMeans I applied StandardScaler — KMeans is distance-based, and unscaled features with larger magnitudes would unfairly dominate the calculation. I then tested k=2 through k=8 using an elbow curve and silhouette scores to choose the right number of clusters.
...
The model was trained on a combined feature matrix of TF-IDF scores and scaled engineered features, with an 80/20 train/test split. It achieved 55% accuracy on held out data 4x better than the 12.5% random chance baseline for 8 classes. The biggest limitation was class imbalance: underrepresented personas like AI-Curious Mama had too few training examples to learn from.
...
- The most common use case was Education & Homework Help — homework support, lesson planning, and breaking down complex concepts for their children
...
📊 [Presentation Summary](https://www.canva.com/design/DAG89D2Xa6Y/IaWOx2B9bSpqVP443emQbA/view)
